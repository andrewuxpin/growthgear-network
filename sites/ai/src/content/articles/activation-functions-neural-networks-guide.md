...
Choosing the right activation function is one of the highest-leverage architectural decisions in deep learning. A wrong choice at the output layer causes silent model failure. A wrong choice in hidden layers can cause gradients to vanish over dozens of layers — an effect rooted in how [backpropagation propagates error signals](/deep-learning/how-does-backpropagation-work-neural-networks) through the chain rule. The activation derivative f'(net) is the same term that appears in the [generalized delta rule](/deep-learning/what-is-the-delta-rule-in-neural-networks), which extends single-layer error-based learning to non-linear networks. Getting it right accelerates training and often improves final accuracy without any other changes.
