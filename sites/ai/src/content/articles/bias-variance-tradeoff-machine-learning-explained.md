...
- **Gradient boosting** (XGBoost, LightGBM): engineered to reduce bias instead. Each new tree is trained specifically to correct the errors of the ensemble so far, which drives training error down aggressively. Left unregularized, that same mechanism will eventually start correcting for noise, so learning rate, tree depth, and early stopping all need active tuning. See our [random forest vs gradient boosting guide](/machine-learning/random-forest-vs-gradient-boosting-guide) for how this bias/variance split plays out in practice across real business use cases.
