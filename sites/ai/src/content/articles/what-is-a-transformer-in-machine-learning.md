...
GPT-3 uses 96 attention heads per layer. The outputs from all heads are concatenated and projected back to the model's working dimension. For a complete technical breakdown of QKV scoring, attention variants (self-attention, cross-attention, Flash Attention), and how context window size is constrained by attention complexity, see our [attention mechanism in deep learning guide](/deep-learning/attention-mechanism-deep-learning-explained). For a focused deep-dive on the parallel-head computation, head count selection, and Multi-Query/Grouped-Query Attention variants, see [what is multi-head attention in deep learning](/deep-learning/what-is-multi-head-attention-in-deep-learning). For a deeper dive into how these language capabilities power business tools, see our guide to [what is natural language processing](/machine-learning/what-is-natural-language-processing-explained).
